...
export const ModalAgregarFuncionarioEvento = ({ evento, onCerrar, onAgregados }: ModalAgregarFuncionarioEventoProps) => {
  ...
  const guardarUno = async () => {
    ...
    setGuardando(true)
    try {
      const trabajador = await db.agregarTrabajadorEventoTransito({
        evento_id: evento.id,
        nombre: nombre.trim(),
      ...
      onAgregados(evento.id, [trabajador as EventoTransitoTrabajador])
    } catch (err) {
  ...
  const guardarMasivo = async () => {
    ...
    setGuardando(true)
    try {
      const trabajadores = await db.agregarTrabajadoresEventoTransito(
        masivoValidos.map((t) => ({ evento_id: evento.id, ...t }))
      )
      onAgregados(evento.id, trabajadores as EventoTransitoTrabajador[])
    } catch (err) {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-lg shadow-xl z-50 p-6 max-h-[85vh] overflow-y-auto">
          ...
          <p className="text-xs text-slate-500 mb-4">
            {evento.tipo === 'subida' ? '▲ Subida' : '▼ Bajada'} suelta del {evento.fecha}
          </p>
